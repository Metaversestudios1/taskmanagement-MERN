...
const ActivityTable = () =>{
 ...
 const handleDelete = async (e,id) => {
    e.preventDefault();
    const activitydelete = window.confirm("Are you sure, you want to delete the activity");
    if(activitydelete){
 ...
 const fetchActivity = async () =>{
    setLoader(true);
    const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/getAllevent?page=${page}&limit=${pageSize}&search=${search}`
    );
    const response = await res.json();
    if(response.success){
        setLoader(false)
 ...
 const startIndex = (page-1)* pageSize;
    ...
    <div className="">
        ...
        <div className="relative overflow-x-auto m-5 mb-0">
            {activity.length > 0 ? <table className="w-full text-sm text-left rtl:text-right border-2 border-gray-300">
            ...
            </table> : <div className="text-center text-xl">Currently! There are no available Holiday.</div>}
        </div>

        {activity.length > 0 && <div className="flex flex-col items-center my-10">
